...
nrBoostersRandom = 3; % max nr of GS that can occur between massed booster CS
maxGSbtwBooster = 2; % after massed booster CS: nr of CS+ with 100% pairing without restrictions of CS/GS order
...
maxdBpossible = 92; % How many dB come out if sound vector is scaled at 1
dBallowed = [91, 88, 32]; % for fixed values

% rating & instruction parameters
pauseBtwRat = .5; % pause duration between two ratings, in sec
buttonsOK = 1; % indices of mouse buttons to select rating
...
logFileFolder = '/home/andreaskeil/Desktop/As_Exps/gaborgentone/logfiles/';
logFilePrefix = 'log'; % TODO: come up with something more specific
ratFileFolder = '/home/andreaskeil/Desktop/As_Exps/gaborgentone/ratings/';
ratFilePrefix = 'rating'; % TODO: come up with something more specific
ratAllFileFolder = '/home/andreaskeil/Desktop/As_Exps/gaborgentone/ratings/';
ratAllFilePrefix = 'ratingall'; % TODO: come up with something more specific
...
welcomeMsg_partStart = ['Everything has been set up now. In the upcoming\n' ...
                        ...
                        'to them. A tone may occur from time\n' ...
                        'to time. Every now and then you will be asked to\n' ...
                        'answer questions about how you experience the shapes.\n'...
...
% check if dB setting okay
if ~ismember(dB, dBallowed)
    error(['Please use one of the allowed settings for [dB]: ' num2str(dBallowed)])
end
...
time = 0:1/soundSamp:1; 
sinewave = sin(2*pi*time*440);
soundIntensity = 10^(log10(1) - ((maxdBpossible-dB)/40));
whiteNoise = [zeros(1,soundSamp*soundOnset), sinewave] .* soundIntensity;
...
try
    ...
    edfFile = ['Ggt' num2str(subNo)];

    calibrate_eyetracker(edfFile, w) % set up and do calibration
   
    hello_eyetracker; % start recording from eye - tracker
    ...
    %send marker to EEG and eyetracker
     
    Eyelink('Message', 'cue_on');
...
%                 whiteNoise, soundOnset, soundSamp, serPort);
        ...
        trialOutVec = [subNo, csPerm, trial, trialMat(trial,4), trialMat(trial,5), ...
                       trialMat(trial,1), trialMat(trial,2), actFlickDur, trialMat(trial,3)];
...
function shuffledMat = pseudoShuffleWithBooster(condVec, trPerCS, nrBoostersMassed, nrBoostersRandom, maxTrBtwBooster, maxSeqCS, percPairing, pairingProbLocal) 
    ...
    elseif pairingProbLocal == false
        nrUSafterBooster = floPlayor(trPerCS*percPairing - nrBoostersMassed - nrBoostersRandom);
    end
...
% [dark bars, background, bright bars]; e.g. [0 127.5 255] for black & white
...
% logged (same order as ratings)
    ...
    textVec = {'How unpleasant is this pattern to you?'; ...
               ...
               'How likely (in %) will this pattern be followed by the tone?'};
